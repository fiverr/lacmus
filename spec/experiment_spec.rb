...
describe Lacmus::Experiment, "Experiment" do
  ...
  describe 'Analytics' do
    ...
    it 'should record timeline data for control version' do
      experiment_id = create_and_activate_experiment.id
      ...
      expect(result.first[1].to_i).to eq(2)
    end

    it 'should record timeline data for experiment version' do
      experiment_id = create_and_activate_experiment.id
      ...
      experiment = Lacmus::Experiment.find(experiment_id)
      result = experiment.kpi_timeline_data(experiment_id, 'ftb', false)
      expect(result.first[1].to_i).to eq(2)
    end
